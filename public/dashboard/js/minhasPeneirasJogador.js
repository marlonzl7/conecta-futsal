async function carregarPeneiras() {
    try {
        const resposta = await fetch(`/peneiras?idJogador=${sessionStorage.ID_JOGADOR}`);
        ...
        for (let i = 0; i < peneiras.length; i++) {
            div.innerHTML += `
                <div class="item">
                    ...
                    <button class="btn btn-inscrever" onclick="redirecionarParaPeneira(${peneiras[i].id_peneira})">Ver detalhes</button>
                </div>
            `;
        }
    } catch (erro) {
